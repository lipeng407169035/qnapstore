...
import { DollarSign, BookOpen, Target, Wrench, BarChart3, Award, Check, Medal } from 'lucide-react';

export default function PartnershipPage() {
  const router = useRouter();
  const benefits = [
    { Icon: DollarSign, title: '具竞争力的利润空间', desc: '提供多层级代理折扣与季度返点奖励，确保合作伙伴有充足的利润空间' },
    { Icon: BookOpen, title: '完整技术培训', desc: '每年至少 4 场产品培训营，涵盖技术认证，销售技巧与售后服务' },
    { Icon: Target, title: '营销资源支持', desc: '提供本地化营销素材、参展补助及数字广告费用补贴' },
    { Icon: Wrench, title: '专属技术支持', desc: '24 小时线上技术客服，优先处理合作伙伴的疑难问题' },
    { Icon: BarChart3, title: 'CRM 系统对接', desc: '提供合作伙伴专用 B2B 平台，支持报价、库存查询与订单管理' },
    { Icon: Award, title: '年度奖励计划', desc: '表现杰出的合作伙伴可获得年度奖金、海外旅游及 QNAP 原厂参访' },
  ];
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 mb-16">
          {benefits.map((b, i) => (
            <div key={i} className="bg-white rounded-2xl p-5 border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-0.5 transition-all">
              <div className="w-12 h-12 bg-blue-light rounded-xl flex items-center justify-center mb-3">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-16">
          {tiers.map((t, i) => (
            <div key={i} className={`rounded-2xl p-6 border-2 ${i === 2 ? 'border-blue bg-blue-50' : 'border-gray-100 bg-white'}`}>
              <div className="w-14 h-14 bg-blue-light rounded-xl flex items-center justify-center mb-3">
                <t.Icon className="w-7 h-7 text-blue" />
              </div>
